...
def ipv6_to_email(boot_flag=0, set_logger_begin=None):
    ...
    if (boot_flag == 0) and (ipv6_data_file in get_path_dirs_files(path=ipv6_data_path)[1]):
        ...
        try:
            ...
            return None
        # 对比两个 IPv6 地址是否一致
        flag = 1 if ipv6_address == file_ipv6_address else 2

    # 若 IP 地址文件未创建(flag == 0)和若主机 IPv6 和 IP 地址文件中两个 IPv6 地址不一致则发送邮件(flag == 2);
    ...
    if (flag == 0) or (flag == 2):
        ...
        try:
            logger.info(
                f"{logger_begin}正在发送邮件" if flag == 0
                else f"{logger_begin}当前系统 IPv6 地址已变化, 正在发送邮件"
            )
            email.send(
